...
const Releases = () => {
  ...
  return (
    <section id="releases" ref={sectionRef} className="w-full bg-[#F0EBE0] py-20 md:py-48 overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-8 md:px-16">
        ...
        <div className="flex items-center gap-6 mb-6">
          ...
          <span className="font-mono text-[11px] tracking-[0.3em] uppercase text-[#4A6090]">Discography</span>
          <div className="flex-1 h-px bg-[#C8C0A8]" />
          <span className="hidden md:block font-mono text-sm text-[#C8C0A8] select-none">ディスコグラフィー</span>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-12">
          <div>
            <h2 className="cs-lg text-[#0C1B4D] leading-[1]">
              Selected<br />
              <span className="text-[#1E3FA8]">Recordings</span>
            </h2>
          </div>
          ...
          {filters.map((f) => (
            <button key={f}
              ...
              className={`font-mono text-[11px] tracking-[0.25em] uppercase px-5 py-2.5 rounded-full border transition-all duration-300 cursor-pointer ${
                activeFilter === f
                  ? 'bg-[#1E3FA8] text-[#F0EBE0] border-[#1E3FA8]'
                  : 'border-[#C8C0A8] text-[#4A6090] hover:border-[#1E3FA8] hover:text-[#1E3FA8]'
              }`}
            >
              {f}
            </button>
        ...
        {loading ? (
          <div className="text-[#1E3FA8] font-mono text-sm animate-pulse tracking-widest uppercase">Loading recordings...</div>
        ) : filteredAlbums.length === 0 ? (
          <div className="text-[#4A6090] font-mono tracking-widest uppercase text-sm">No releases found.</div>
        ) : (
            ...
            {filteredAlbums.map((album, index) => {
              ...
              return (
                <div key={album.id} ref={(el) => (rowsRef.current[index] = el)}
                  className="group grid grid-cols-12 gap-4 md:gap-8 items-center py-8 md:py-10 border-b border-[#C8C0A8] hover:bg-[#1E3FA8]/5 transition-colors duration-500 cursor-pointer px-2 md:px-4"
                ...
                >
                  ...
                  <div className="col-span-2 md:col-span-1">
                    <span className="font-mono text-[11px] tracking-[0.3em] text-[#1E3FA8]">
                  ...
                  <div className="col-span-6 md:col-span-5">
                    <h3 className="cs-sm text-[#0C1B4D] leading-none group-hover:text-[#1E3FA8] group-hover:translate-x-2 transition-all duration-500">
                      {album.title}
                    </h3>
                  </div>
                  ...
                  <div className="hidden md:block col-span-2">
                    <span className="font-mono text-[10px] tracking-[0.25em] uppercase text-[#4A6090]">
                      {album.type || 'ALBUM'}
                    </span>
                  </div>
                  ...
                  {/* Year */}
                  <div className="hidden md:block col-span-2">
                    <span className="font-mono text-xs text-[#4A6090]">
                  ...
                  <div className="col-span-4 md:col-span-2 flex items-center justify-end gap-3">
                    {album.spotify_url && (
                      <a href={album.spotify_url} target="_blank" rel="noreferrer"
                        ...
                        className="text-[#4A6090] hover:text-[#1E3FA8] transition-colors" title="Spotify">
                    ...
                    {album.apple_music_url && (
                      <a href={album.apple_music_url} target="_blank" rel="noreferrer"
                        onClick={(e) => e.stopPropagation()}
                        className="text-[#4A6090] hover:text-[#1E3FA8] transition-colors" title="Apple Music">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="w-10 h-10 rounded-full border border-[#C8C0A8] flex items-center justify-center group-hover:bg-[#1E3FA8] group-hover:border-[#1E3FA8] transition-all duration-300">
                      <ArrowUpRight size={16} className="text-[#4A6090] group-hover:text-[#F0EBE0] transition-colors duration-300" />
                    </div>
                  </div>
                </div>
              );
