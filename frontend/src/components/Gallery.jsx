...
const Gallery = () => {
  ...
  return (
    <section ref={sectionRef} id="gallery" className="w-full h-screen bg-[#04060A] overflow-hidden flex items-center relative">
      ...
      {loading ? (
        <div className="w-full flex justify-center text-[#8FA9C4] font-mono text-sm animate-pulse tracking-widest uppercase">
          Loading memory...
        </div>
      ) : images.length === 0 ? (
        <div className="w-full flex justify-center text-[#8FA9C4] font-mono tracking-widest uppercase text-sm">
          No photos available.
        </div>
      ) : (
          ...
          {images.map((imgObj, index) => {
            ...
            return (
              <figure key={imgObj.id}
                className={`relative shrink-0 bg-[#1A365D] border border-[#3A609E] overflow-hidden group ${mt}`}
              ...
              >
                ...
                <div className="p-4 flex justify-between items-center">
                  <figcaption className="font-mono text-sm text-[#F0EBE0] leading-none">
                    {imgObj.caption || `Frame ${String(index + 1).padStart(2, '0')}`}
                  </figcaption>
                  <span className="font-mono text-[9px] tracking-[0.2em] uppercase text-[#8FA9C4]">
